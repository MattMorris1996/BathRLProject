...
class TD3:
    ...
    def __init__(self, env, seed):
        ...
        self.critic2 = self.create_model((self.env.observation_space.shape[0], self.env.action_space.shape[0]))
        self.target_critic = self.create_model((self.env.observation_space.shape[0], self.env.action_space.shape[0]))
        self.target_critic2 = self.create_model((self.env.observation_space.shape[0], self.env.action_space.shape[0]))
        self.target_actor.set_weights(self.actor.get_weights())
        self.target_critic.set_weights(self.critic.get_weights())  # ensure initial weights are equal for networks
        self.target_critic2.set_weights(self.critic.get_weights())
        self.reset()
        # return self.actor
    ...
    def replay_crit(self, states, actions, rewards, next_states):  # , actor, target_actor, critic, target_critic):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            ...
            q_current1 = self.critic([states, actions])
            q_current2 = self.critic2([states, actions])

            crit1_loss = tf.math.reduce_mean(tf.math.square(q_current1 - q_target_value))
            crit2_loss = tf.math.reduce_mean(tf.math.square(q_current2 - q_target_value))

            # crit1_loss = tf.keras.losses.MSE(q_target_value, q_current1)
            # crit2_loss = tf.keras.losses.MSE(q_target_value, q_current2)
        ...
        self.critic.optimizer.apply_gradients(zip(grad1, self.critic.trainable_variables))
    ...
    def train(self, state, action, reward, next_state, terminal, steps):
        ...
        if steps % 2 == 0:  # Update targets only every X steps
            self.train_target()
